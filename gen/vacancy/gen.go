...
func main() {
	...
	structTypeResumeSearch, ok := objVacancySearch.Type().Underlying().(*types.Struct)
	if !ok {
		log.Fatalf("type %v is not a struct", objVacancySearch)
	}

	err = generateVacancyFindMethods("pb", pkgPath, "VacancySearchEntity", structTypeVacancy, structTypeResumeSearch)
	if err != nil {
		log.Fatal(err)
	}
